...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<Either<Failure, DefaultEntity>> getAdminContact(
    NoParams noParams,
  ) async {
    try {
      final remoteAdminContact =
          await remoteDataSource.getAdminContact(noParams);

      return Right(remoteAdminContact);
    } on DioError catch (e) {
      // The request was made and the server responded with a status code
      // that falls out of the range of 2xx and is also not 304.
      if (e.response != null) {
